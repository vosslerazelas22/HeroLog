        ...
        <div className="space-y-3 w-full">
          {skills.map((sk, idx) => {
            ...
            return (
              <div
                key={idx}
                className="bg-stone-950/40 border border-zinc-800 p-3 rounded hover:border-zinc-700/60 transition-all space-y-2 w-full"
              >
                      ...
                      {isEditing ? (
                        <div className="flex items-center gap-1.5 w-full">
                          <input
                            ...
                            className="bg-stone-950 border border-zinc-700 rounded px-2 py-0.5 text-xs text-zinc-100 focus:outline-none focus:border-champagne-400 w-full"
                            maxLength={30}
                            autoFocus
                          />
                          <button
                        ...
                        <div className="space-y-1">
                          <div className="flex items-center gap-1.5 flex-wrap">
                            <strong className="text-zinc-100 font-serif text-sm leading-tight break-words">{sk.name}</strong>
                            ...
                            <button
                              ...
                              className="p-1 text-zinc-500/50 hover:text-zinc-300 rounded cursor-pointer shrink-0 transition-all"
                          ...
                          <div className="flex items-center gap-2">
                            <span className="bg-champagne-500/10 border border-champagne-500/20 text-champagne-400 font-bold font-mono text-[9px] px-1.5 py-0.5 rounded">
                              Nível {sk.level}
                            </span>
                          </div>
                        </div>
                ...
                <div className="pt-1.5 border-t border-zinc-800 space-y-1.5">
                  <div className="flex justify-between items-center text-[9px] uppercase font-serif tracking-wider text-zinc-500">
                    <span>Subskills:</span>
                  </div>
                  {(!sk.tags || sk.tags.length === 0) ? (
                    <div className="text-[9px] text-zinc-500 italic">Nenhuma subskill cadastrada para esta habilidade.</div>
                  ) : (
                    <div className="flex flex-wrap gap-1">
                      {sk.tags.map((tg, tIdx) => (
                        <span key={tIdx} className="px-1.5 py-0.5 bg-white/5 border border-zinc-700 text-zinc-300 text-[9px] rounded flex items-center gap-1 font-sans">
                          {tg}
                          <button
                            type="button"
                            onClick={() => onRemoveTagFromSkill(idx, tIdx)}
                            className="text-zinc-500 hover:text-red-400 font-extrabold ml-0.5 cursor-pointer text-[9px]"
                  ...
                  <div className="flex gap-1">
                    <input
                      ...
                      className="flex-1 bg-stone-950/50 border border-zinc-800 rounded px-2 py-0.5 text-[10px] text-zinc-200 placeholder-zinc-600 focus:outline-none focus:border-champagne-400 font-sans"
                    />
                    <button
                      type="button"
                      onClick={() => {
                      ...
                      className="px-2 py-0.5 bg-champagne-500/15 hover:bg-champagne-500/25 border border-champagne-500/20 text-champagne-300 hover:text-champagne-100 text-[10px] font-bold rounded cursor-pointer transition-all"
                    >
                      +
                    </button>
                  </div>
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                className="px-4 py-2 bg-champagne-500/15 border border-champagne-400 text-champagne-300 text-xs font-serif uppercase tracking-widest hover:bg-champagne-400 hover:text-stone-950 rounded transition-all cursor-pointer shrink-0"
              >
                Gravar
              </button>
            </div>
